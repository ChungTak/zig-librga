...
fn buildExamples(
    ...
    const examples = [_]struct { name: []const u8, path: []const u8 }{
        .{ .name = "rgaIm_demo", .path = "src/examples/rgaIm_demo.zig" },
    };
    ...
    for (examples) |example| {
        ...
        // 设置example的librga依赖
        setupZrgaForExecutable(exe, lib_path, include_path, zrga_module);
...
pub fn build(b: *std.Build) void {
    ...
    if (!is_supported) {
        std.debug.print("Error:zrga does not support the  {s}  target.\n", .{target_str});
        return;
    }

    // 获取LIBRGA库路径LIBRGA_LIBRARIES环境变量
    ...
    // 创建库模块
    const zrga_module = createZrgaModule(b, target, optimize, b.path(final_lib_path), b.path(final_include_path));
